...
def add_slide_block(title, lines):
    p = doc.add_paragraph()
    r = p.add_run(title)
    r.bold = True
    r.font.size = Pt(12)
...
doc.add_heading("IoT Midterm Presentation Script", level=1)
p = doc.add_paragraph(
...
doc.add_paragraph("Slides: 아이오티 중간발표 체크.pptx (10 slides)")
doc.add_paragraph("Target time: ~3 minutes (+ ~2 min Q&A)")
doc.add_paragraph("")

doc.add_heading("Part 1 — English Script (3 minutes)", level=2)

en_slides = [
    ("[Slide 1 — Title · 0:00–0:15]", [
        "Good morning/afternoon, Professor.",
        "We are Team Park Hyodam and Jo Hyeonjin from IoT Systems.",
        "Our project is Motion-Adaptive AI Companion Robot — an LLM-based Emotional Interactive AIoT System on Raspberry Pi.",
    ]),
    ("[Slide 2 — A. Main Idea & Objective · 0:15–0:40]", [
        ...
        "We build one AIoT pipeline on a single device: Sensor → Decision → Actuator.",
        "For sensing, we use a camera and an ultrasonic sensor. For output, we use LCD, LED, and buzzer.",
    ...
    ("[Slide 5 — D. Hardware · 1:30–1:50]", [
        ...
        "The camera uses CSI and rpicam-still. The LCD uses I2C.",
        "The ultrasonic uses TRIG on GPIO 16 and ECHO on GPIO 18.",
        "The LED is on GPIO 17. The buzzer is on GPIO 27.",
    ...
    ("[Slide 7 — F. Project Understanding · 2:10–2:30]", [
        ...
        "We plan a state flow: Idle when no user is detected, Approach when distance is within 20 cm,",
        'Stay when the user remains nearby and we ask "Want to talk?", Alert for sudden large motion, and Return to idle after timeout.',
...
for title, lines in en_slides:
    add_slide_block(title, lines)

doc.add_heading("English — Timing Guide", level=3)
add_table(
    ["Slide", "Topic", "Time"],
    [
        ["1", "Title", "0:15"],
...
doc.add_heading("Part 2 — 한국어 대본 (3분)", level=2)

ko_slides = [
    ("[슬라이드 1 — 제목 · 0:00–0:15]", [
        "안녕하세요, 교수님. IoT Systems 수업 중간 발표입니다.",
        "팀장 박효담, 팀원 조현진입니다.",
        "프로젝트 제목은 Motion-Adaptive AI Companion Robot, LLM 기반 감정형 AIoT 동반 로봇입니다.",
    ]),
    ("[슬라이드 2 — A. 주제와 목표 · 0:15–0:40]", [
        "핵심 아이디어는 사용자를 감지하고 감정적으로 반응하는 데스크톱 동반 로봇입니다.",
        "라즈베리파이 한 대에서 센서 → 판단 → 출력 AIoT 파이프라인을 구현합니다.",
        "센서는 카메라와 초음파 센서, 출력은 LCD, LED, 버저, 상호작용은 2×16 LCD에 짧은 영어 응답을 표시합니다.",
    ...
    ("[슬라이드 3 — B. 시스템 설계 · 0:40–1:05]", [
        ...
        "2초마다 rpicam-still로 프레임을 촬영하고, detect_motion()으로 중앙 ROI 프레임 차이를 확인합니다.",
        "초음파로 20cm 이하 거리도 함께 검사합니다. 움직임 또는 근접이 감지되면 show_motion()을 호출합니다.",
    ]),
    ("[슬라이드 4 — C. 핵심 코드 · 1:05–1:30]", [
        "핵심은 lcd_actuator.py의 Actuator 클래스입니다.",
        ...
        "처음에는 led_lcd.txt 초안으로 LCD와 LED만 구현했고, 지금은 show_motion()에 버저까지 통합했습니다.",
    ]),
    ("[슬라이드 5 — D. 하드웨어 · 1:30–1:50]", [
        "하드웨어는 라즈베리파이 4와 다섯 가지 주변장치입니다.",
        "카메라는 CSI, LCD는 I2C, 초음파는 TRIG 16번·ECHO 18번, LED는 GPIO 17번, 버저는 GPIO 27번입니다.",
        "ECHO가 18번이라 버저와 핀이 겹쳐서, 버저를 27번으로 분리했습니다.",
    ...
    ("[슬라이드 7 — F. 프로젝트 이해 · 2:10–2:30]", [
        ...
        "상태 흐름을 설계했습니다. Idle — 사용자 없음, Approach — 20cm 이하 접근,",
        'Stay — 근처에 머무름, "Want to talk?" 유도, Alert — 갑작스러운 큰 움직임, Return — 입력 없으면 idle 복귀.',
        "핵심은 사용자 상황을 상태로 분류하고 감정형 반응을 선택하는 것입니다.",
    ]),
    ...
    ("[슬라이드 9 — 향후 계획 · 2:45–2:55]", [
        ...
        "3단계: LCD 감정형 응답 로직.",
        "최종 목표는 움직임·거리 감지 → 감정 상태 → LCD·LED·버저·짧은 LLM 응답입니다.",
    ]),
...
for title, lines in ko_slides:
    add_slide_block(title, lines)

doc.add_heading("한국어 — 슬라이드별 시간표", level=3)
add_table(
    ["슬라이드", "내용", "시간"],
    [
        ["1", "제목", "0:15"],
...
doc.add_heading("발표 팁 / Presentation Tips", level=2)
for tip in [
    "슬라이드 6~7이 길면 슬라이드 8을 짧게 줄이세요.",
    "PPT는 영어, 말은 한국어 대본으로 해도 됩니다. 영어 발표 시 Part 1 사용.",
    "시연 영상이 있으면 슬라이드 5 또는 6에서 Lab 9 demo 언급 가능.",
    "Q&A 예상: Why camera + ultrasonic? / Why no speaker? / Is LLM on Pi?",
]:
    doc.add_paragraph(tip, style="List Bullet")

out = r"d:\26-1\Iot\Project\docs\midterm\MIDTERM_SCRIPT.docx"
doc.save(out)
print("Saved:", out)
