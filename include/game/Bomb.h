...
struct BombTuning {
  static constexpr float minDragPixels = 14.0f;
  static constexpr float throwStrength = 9.0f;
  static constexpr float minThrowSpeed = 90.0f;
  static constexpr float maxThrowSpeed = 540.0f;
...
public:
  ...
  Vec2 PreviewVelocity(const InputState& input) const;

  void TryThrow(const InputState& input, PhysicsWorld& world, const Body& player);
